...
**`src/App.tsx`** — Root component; composes top-level state from custom hooks (`useOverlayStack`, `useFileEditor`, `useWorkspaceSession`, `useGit`, `useGitUiController`, `useFilesMode`, `useWorkspaceRevisionPoll`, `useGitFocusFetch`, `useMenuActions`, `useKeyboardShortcuts`, `useGitRefreshOnSave`, `useTheme`, `useToast`, `useEditorPreferences`, `useWordCountGoal`, `useRecentWorkspaces`, `useCollectionLinks`) and owns only the few UI flags that aren't overlays (sidebar visibility, zen/typewriter mode, cover image visibility). Every dialog/popover visibility lives in one tagged-union owned by `useOverlayStack`. Render output is composed from four top-level components: `Sidebar`/`SearchPanel` (left column), `EditorPane` (editor + properties), `StatusBar`, and `AppDialogs` (all overlays). `handleSidebarSelect` routes non-markdown selections to `FileViewer`; `closeActiveTabOrFile` closes the FileViewer first when it is active. `openFileByPath` is a thin wrapper that clears `fileViewerNode` and delegates to `useWorkspaceSession.openByPath` — the session enforces the "select + push recent + open tab" invariant so callers don't have to assemble it.
...
- `AppDialogs.tsx` — All overlay/dialog rendering; 14 lazily-imported dialog components (`WorkspaceSwitcher`, `FileSwitcher`, `NewFileDialog`, `AddToCollectionDialog`, `CommitDialog`, `BranchSwitcher`, `NewBranchDialog`, `RenameBranchDialog`, `DeleteBranchDialog`, `GitSyncPopover`, `PerfPanel`, `UpdateDialog`, `RenamePathDialog`, `WechatPublishDialog`) plus the toast container. Consumes the `OverlayStack` directly and renders by switching on `overlay.active?.kind` / `overlay.is(kind)`; visibility is never duplicated as a prop.
- `Editor.tsx` — Tiptap WYSIWYG editor; StarterKit + Markdown + Typography + Link + Table (+ TableCell/Header/Row) + Mathematics + Placeholder + CodeBlockLowlight + TaskList/TaskItem (from `@tiptap/extension-list`) + custom extensions in `src/lib/tiptap/`. Keyboard shortcuts and native menu actions are dispatched from a single declarative table — see `src/lib/editor/commands.ts` + `useEditorCommands` (ADR 0008).
...
- `useOverlayStack.ts` — one tagged-union owner for every dialog/popover visibility (modal, switcher, workspaceSwitcher, search, update, wechatPublish, shortcutsHelp, commit, branchSwitcher, newBranch, renameBranch, deleteBranch, gitSyncPopover). Exposes `active` / `is(kind)` / `open(overlay)` / `close(kind?)` / `isBlocking`. See [ADR 0006](docs/adr/0006-overlay-stack-tagged-union.md) and the CONTEXT.md "Overlay model" section.
- `useWorkspaceSession.ts` — coordinator that composes `useWorkspace` + `useEditorSession` and owns the `sessionRef` bridge between them. The previous workaround that exposed the cycle at the App.tsx top level is encapsulated here. Returns the union of both hooks' APIs. See [ADR 0007](docs/adr/0007-workspace-session-coordinator.md).
- `useWorkspace.ts` — workspace open/close, the canonical recursively-walked tree (from `workspace_root`, with noise dirs filtered Rust-side), `flatFiles: FlatFile[]` (markdown-only projection used by `Cmd+P` and `openFileByPath`), `postsBasePath` (from `site.config.ts`), and the file-lifecycle handlers (`handleNewFile`, `handleRename`, `handleDelete`, `addCollectionItem`, `removeCollectionItem`, etc.). `handleNewFile` builds content via `amytisScaffold.buildNewContent` (Amytis-native paths/frontmatter, see ADR 0009); the collection handlers read-modify-write a collection index's `items:` via `collection.ts`. Lifecycle handlers fire `onPathCreated` / `onPathRenamed` / `onPathRemoved` callbacks so the editor session stays in sync without `useWorkspace` knowing about tabs, recents, or selection. Consumed via `useWorkspaceSession` from `App.tsx`.
- `useCollectionLinks.ts` — for each collection entry (`findCollectionEntries`), reads its `index` `items:` and resolves them to `CollectionLink[]` (keyed by collection dir path). Reads are keyed on the *set* of collection paths (not tree identity) so the revision poll doesn't re-read every tick; `reload()` after add/remove. Composed in `App.tsx`.
- `useFileEditor.ts` — current file content, dirty tracking, auto-save coordination, frontmatter parse. Owns the save path. `App.tsx` calls it directly for editor-pane state (`fileContent`, `wordCount`, `parsedFrontmatter`, `saveStatus`), then passes a slim `FileEditorHandle` (`selectedFile`, `selectedPathRef`, `setSelectedFile`, `handleSelectFile`, `handleCloseFile`) into `useWorkspaceSession` so the open-file orchestration stays inside `useEditorSession`.
- `useEditorSession.ts` — the answer to "which file is the user editing right now, and what is its history?". Composes `useFileEditor` (passed in to break the dependency cycle with `useWorkspace`), `useOpenTabs`, and `useRecentFiles`. Owns `openFile(node)` / `openByPath(path)` (select + push recent + open tab as one step), `closeActive` (close tab and advance to neighbour, or close editor entirely), `notifyPathRenamed(old, new, lookup?)` and `notifyPathRemoved(path)` (keep tabs + recents + selection in lockstep on filesystem mutations). The pure helpers `selectionAfterRename` and `selectionShouldCloseAfterRemove` are exported for testing. Consumed via `useWorkspaceSession` from `App.tsx`.
- `useGit.ts` — git state (branch, status, remotes); polls via Tauri commands
- `useGitUiController.ts` — coordinates git dialogs (commit, branch CRUD, sync popover). Dialog *visibility* lives in the overlay stack (passed in as the `overlay` option); the controller owns dialog *data* (commit changes, branch lists) and constructs the overlay payload via `overlay.open({ kind: "commit", state: { ... } })`.
...
- `useKeyboardShortcuts.ts` — wires all global `keydown` shortcuts to app-level state and actions; takes `overlay: OverlayStack` and uses `overlay.isBlocking` to suppress shortcuts when a dialog is open
...
- `useMenuActions.ts` — subscribes to the native `menu-action` Tauri event (via `listenEvent` from the typed seam) and dispatches each *app-level* action to the appropriate handler; also owns `handleWechatCopy`. Takes `overlay: OverlayStack` and uses `overlay.isBlocking` for the suppress check. Editor-routed menu actions (`format-*`, `insert-*`) are handled separately by `useEditorCommands` inside the editor.
- `useContentTypes.ts` — Amytis content type discovery (parses `contentTypes` from `site.config.ts`). Currently **not consumed by the UI**: content type is derived from the bucket folder (see ADR 0009 / CONTEXT.md), so new-content typing no longer reads this. Kept as infra.
...
- `sidebarUtils.ts` — `forContentMode(tree, { workspaceRoot, treeRoot, postsBasePath })` and `forFilesMode(tree)` are the two top-level mode projections applied in `App.tsx`. `forContentMode` keeps series/book **entry folders** as directories (only folder-backed posts collapse). Exported helpers: `getBucketContentType` (bucket folder → content type, honoring `postsBasePath`), `getDirIndexEntry`, `isCollectionEntry`, `findCollectionEntries`, `getSidebarDisplayName`, `needsPageDivider`. Internal: `collapseIndexNodes`, `sortTree`/`sortNodes` (content-type priority), `sortTreeAlpha`, `rollupGitStatus`, `filterTree`. Noise-dir filtering moved to the Rust walk.
- `amytisScaffold.ts` — `buildNewContent(input)` reproduces the Amytis `new-*` scripts: per-`NewContentKind` path/extension/date-prefix/frontmatter, reading `templates/default.mdx` for posts. No file it writes carries a `type:` field. See ADR 0009.
- `collection.ts` — collection `items:` model: `parseCollectionItems` / `resolveCollectionItems` (slug → `CollectionLink`), `collectionCandidates` (Add picker), `addItem` / `removeItem` / `itemKey` / `postSlugOf`, and `setCollectionItems` (write `items:` back, preserving other frontmatter). See ADR 0010.
...
**`src/lib/editor/`** — editor command dispatch (ADR 0008)
- `commands.ts` — declarative table of every editor-routed command (keyboard shortcuts + native menu actions), plus the pure helpers `getEditorCommandById`, `shortcutMatches`, `commandCanRun`. Also hosts the private `pickAndInsertImage` helper used by the `insert-image` command. Adding a new editor command is one row here + (optionally) one entry in `src-tauri/src/menu.rs`.
- `useEditorCommands.ts` — two `useEffect`s (keyboard, menu) that dispatch from the table. Consumed by `Editor.tsx`.
...
- `open_workspace` (folder picker) / `open_workspace_at_path` — async, tokio oneshot; performs the canonical recursive `walk_tree` from `workspace_root` and returns the full tree in one round trip. `WorkspaceResult` also carries `cdnBase`, `defaultAuthor`, and `postsBasePath` (the latter parsed by `parse_posts_base_path` in `content_types.rs`)
...
- **`useEditorSession` owns the open-file lifecycle** — the editor session (current file + tabs + recents + selection) is a single hook composing `useFileEditor` / `useOpenTabs` / `useRecentFiles`. `useWorkspace` notifies the session via `onPathCreated` / `onPathRenamed` / `onPathRemoved` callbacks; it never reaches into editor state directly. (ADR 0003)
- **`useWorkspaceSession` encapsulates the cycle** — the `sessionRef` bridge between `useWorkspace` (needs callbacks) and `useEditorSession` (built after the workspace) lives inside one coordinator hook instead of at the App.tsx top level. App.tsx composes the workspace+session in a single hook call. (ADR 0007)
- **One tagged-union owns every overlay** — `useOverlayStack` replaces twelve independent visibility booleans and the duplicated "is anything blocking?" check that previously lived in both `useKeyboardShortcuts` and `useMenuActions`. The rule "only one overlay can be active at a time" is enforced by the type, not by convention. (ADR 0006)
- **Keyboard shortcuts have a single source of truth** — `src/lib/shortcuts.ts` lists every shortcut (global + editor-command-table + Tiptap defaults) with a stable id, key combo, category, and i18n description key. The in-app Keyboard Shortcuts help dialog (`?` or Help → Keyboard Shortcuts) renders from this module; `docs/shortcuts.md` is hand-maintained to match. The Tiptap-conflict test in `src/lib/editor/commands.test.ts` runs every command-table shortcut through `bindingConflictsWithTiptap` to catch the ProseMirror shift-letter normalization trap (the original `Cmd+Shift+I` bug class) before it ships.
- **Editor commands are a declarative table** — every keyboard shortcut and native menu action dispatches from one table in `src/lib/editor/commands.ts`, via the `useEditorCommands` hook. Adding a new command is one row plus an optional entry in `src-tauri/src/menu.rs`; keyboard and menu surfaces cannot drift apart. (ADR 0008)
- **Content creation mirrors Amytis** — content type is derived from the bucket folder (no `type:` frontmatter); `amytisScaffold.buildNewContent` reproduces the Amytis `new-*` scripts (paths, `.mdx`/`.md`, date-prefixed posts, per-type frontmatter, `templates/default.mdx`); `posts.basePath` is read from `site.config.ts`. (ADR 0009)
- **Collections are link views** — a `type: collection` series references posts/series via an `items:` list; Ovid renders those as navigable links and edits the list in place rather than treating them as in-folder files. (ADR 0010)
