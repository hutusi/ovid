...
- **`src/components/`** — UI components. All overlays/dialogs are rendered by `AppDialogs.tsx`; dialog *visibility* is never a prop — it lives in `useOverlayStack`. Non-markdown files selected in Files mode preview via `FileViewer.tsx`.
