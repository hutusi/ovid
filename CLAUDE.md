...
- **`src/lib/editor/`** — declarative table of every editor-routed command (keyboard + native menu), dispatched by `useEditorCommands`. Adding a command is one row + an optional `src-tauri/src/menu.rs` entry. See [ADR 0008](docs/adr/0008-editor-command-table.md). Also home to the editor's extracted orchestration: `useMarkdownSync` (debounced serialization + flush) and `imagePaste` (paste/drop image handlers as injected factories).
...
- **Keyboard shortcuts have a single source of truth** — `src/lib/shortcuts.ts` lists every shortcut (global + editor-command-table + Tiptap defaults). The in-app help dialog renders from it; `docs/shortcuts.md` is hand-maintained to match. Global shortcuts and native menu actions *dispatch* through one declarative table in `src/lib/appActions.ts` (action id == menu payload id; keyboard entries join by id) — see [ADR 0018](docs/adr/0018-global-action-dispatch-table.md). The conflict test in `src/lib/editor/commands.test.ts` guards the ProseMirror shift-letter normalization trap (the original `Cmd+Shift+I` bug class).
...
- **Use the `<Modal>` primitive** — all *modal* dialogs render through `src/components/Modal.tsx` (+ `<ModalActions>` for the Cancel/Confirm footer), which owns the overlay/backdrop/panel structure, `useFocusTrap`, dialog ARIA, Escape (with `stopPropagation`), and backdrop-click close; see [ADR 0017](docs/adr/0017-shared-modal-primitive.md). Pass dialog-specific keys (Enter-submit, list navigation) via the `onKeyDown` prop — it runs before the Escape handler; when Enter submits, check `e.target === inputRef.current` to avoid double-firing when focus is on a button. The one deliberate exception is the compact properties drawer (`PropertiesPanel` in drawer mode) — a **non-modal** `role="dialog"` that keeps the background interactive, so it omits `aria-modal` and the focus trap, using `useNonModalDialogFocus` (focus in on open, return on close) instead; see [ADR 0004](docs/adr/0004-plain-css-modals-no-portal.md)
