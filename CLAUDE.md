...
- `useOverlayStack.ts` — one tagged-union owner for every dialog/popover visibility (modal, switcher, workspaceSwitcher, search, update, wechatPublish, commit, branchSwitcher, newBranch, renameBranch, deleteBranch, gitSyncPopover). Exposes `active` / `is(kind)` / `open(overlay)` / `close(kind?)` / `isBlocking`. See [ADR 0006](docs/adr/0006-overlay-stack-tagged-union.md) and the CONTEXT.md "Overlay model" section.
