...
describe("buildNewContent", () => {
  it("post: date-prefixed flat .mdx in the basePath bucket", () => {
    const out = buildNewContent({ ...base, kind: "post", title: "My Great Post" }, localT);
  ...
  it("series: folder-backed index.mdx with an images dir", () => {
    const out = buildNewContent({ ...base, kind: "series", title: "Next.js Deep Dive" }, localT);
  ...
  it("seriesPost: flat .mdx inside the series folder, no date prefix", () => {
    const out = buildNewContent({ ...base, kind: "seriesPost", title: "Part 3" }, localT);
  ...
  it("note: flat .mdx with tags and aliases", () => {
    const out = buildNewContent({ ...base, kind: "note", title: "Tailwind v4" }, localT);
  ...
  it("escapes double quotes in titles to keep YAML valid", () => {
    const out = buildNewContent({ ...base, kind: "note", title: 'A "quoted" title' }, localT);
  ...
  it("note: md format produces a flat .md file", () => {
    const out = buildNewContent(
      { ...base, kind: "note", title: "Tailwind v4", format: "md" },
      localT
    );
