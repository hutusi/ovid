...
80. ✅ **Bi-directional wiki links** — Obsidian-style `[[Target]]` (and `[[Target|Display]]`) inline node with markdown round-trip; resolution against notes' `title:` / `aliases:` frontmatter; first-click auto-creates `notes/<slug>.md` via the existing scaffold; "Linked references" backlinks panel at the bottom of the editor scroll area. See [ADR 0016](docs/adr/0016-bidirectional-wiki-links.md).
