...
Tauri/WebView constraints matter here: avoid portal-based UI libraries and prefer native HTML plus plain CSS. Modal dialogs render through the shared `<Modal>` primitive (`src/components/Modal.tsx`, ADR 0017), which owns the focus trap, dialog ARIA, and Escape handling. The compact properties drawer is the one deliberate non-modal `role="dialog"` exception (no `aria-modal`, no focus trap; `useNonModalDialogFocus` instead — ADR 0004). Use Tauri commands or plugins for file access; do not introduce Node-style filesystem APIs into the frontend.
...
`[[Target]]` / `[[Target|Display]]` wiki links are a Tiptap inline atom node (`src/lib/tiptap/WikiLink.ts`) round-tripped through tiptap-markdown via `addStorage.markdown` (parse via a markdown-it inline rule, serialize verbatim). Resolution is alias-first then title (case-insensitive, scoped to `notes/`); no filename-slug fallback. An unresolved target only writes `notes/<slug>.md` on first navigation, via `buildNewContent("note", …, { format: "md" })`. Inbound references render in a "Linked references" section at the bottom of the editor scroll, fed by the `findBacklinks` scanner. See ADR 0016 for the full rationale, including why we picked a node over a decoration and the `[[`-suggestion popover design.
